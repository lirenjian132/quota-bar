...
### Added
- **基元律动（TokenRhythm）平台：余额型多账号监控**。查询网页端用户中心同款接口 `GET /api/wallet/summary`，鉴权用登录会话 `tr_session` cookie（当作凭据存入 FileKeyStore，0600 加密）；每账号显示整数余额（四舍五入），低于 10 元状态栏变红提示换号
...
- 凭据即"登录一次抄一次"：浏览器登录 → 从 DevTools/CDP 抄 `tr_session` 值（约 30 天有效）→ 粘贴到账号配置框即可；**绝不能在网页点「退出登录」**（会在服务端作废会话），换号采集用"本地清 cookie + 直接登录下一账号"流程
...
- 右键菜单「显示指标」可选项按平台分组（此前为全局硬编码列表，余额型平台会得到空子菜单）

## [2.2.0] - 未发布

### Added
- StepFun（阶跃星辰）套餐显示：实现过一版（网站内部接口 + Oasis-Token，参照 CodexBar 逆向文档），因官方无公开 API、token 需手动维护、上手复杂，应用户要求移除；完整实现保留在 git 历史，待官方开放公开 API 后可直接捞回
...
### Fixed
- **API key 存储改用应用私有文件（0600），彻底消除钥匙串登录密码弹窗**：ad-hoc 签名应用无法获得钥匙串持久授权，此前每次启动/更新构建都弹密码；现启动时自动从 Keychain 迁移（仅最后一次授权），此后不再触碰 Keychain
